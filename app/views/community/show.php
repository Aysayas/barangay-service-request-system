...
<section class="workflow-page">
        ...
        <div>
            ...
            <div class="mt-4 flex flex-wrap items-center gap-3 text-sm">
                ...
                <?php if (!empty($post['published_at'])): ?>
                    <span class="text-slate-600">Published <?= e(date('M d, Y', strtotime($post['published_at']))); ?></span>
                <?php endif; ?>
            </div>
        </div>
    ...
    <div class="mt-6 grid gap-6 lg:grid-cols-[1fr_0.35fr]">
        <article class="article-panel">
            ...
            <div class="article-content">
                <?php if (!empty($post['excerpt'])): ?>
                    <p class="text-base leading-7 text-slate-700"><?= e($post['excerpt']); ?></p>
                <?php endif; ?>
        ...
        <aside class="space-y-6">
            <?php if ($post['category'] === 'event'): ?>
                <section class="workflow-card">
                    ...
                    <dl class="mt-4 space-y-4 text-sm">
                        <div>
                            <dt class="font-medium text-slate-800">Schedule</dt>
                            <dd class="mt-1 text-slate-600"><?= e(community_event_schedule($post) ?: 'To be announced'); ?></dd>
                        </div>
                        <div>
                            <dt class="font-medium text-slate-800">Venue</dt>
                            <dd class="mt-1 text-slate-600"><?= e($post['venue'] ?: 'To be announced'); ?></dd>
                        </div>
                        <div>
                            <dt class="font-medium text-slate-800">Organizer</dt>
                            <dd class="mt-1 text-slate-600"><?= e($post['organizer'] ?: 'Barangay Office'); ?></dd>
                        </div>
                    </dl>
                </section>
            <?php endif; ?>
            ...
            <section class="workflow-card">
                <h2 class="text-lg font-semibold text-slate-950">Related Items</h2>
                <?php if (empty($related_posts)): ?>
                    <p class="mt-3 text-sm text-slate-600">No related community posts yet.</p>
                <?php else: ?>
